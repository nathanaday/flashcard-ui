# Networking Exam Review Guide — Combined Question Bank
...
*Source: 2020 #4, 2021 #4 (minor wording variant: "if" vs "as long as"; same intent and key).*
...
*Source: 2020 #5 (key: True), 2021 #5 (key: False).*

Key discrepency in 2020 is disregarded, false is the more defensible answer
...
> Unlike Ethernet, wireless nodes may not be able to transmit and listen at the same time making collision detection impossible.

**Answer: True**

*Source: 2021 #8.*

Wireless radios typically operate half-duplex on a single channel — the transmitter's own outgoing signal would overwhelm any incoming signal at the same antenna, so a node cannot reliably listen for collisions while transmitting. This is the underlying reason 802.11 uses collision *avoidance* (CSMA/CA) with explicit ACKs rather than 802.3's collision detection (CSMA/CD).
...
> In TCP, A source's retransmission timeout value (RTO) is always set equal to the most-recently measured RTT

**Answer: False**

*Source: 2021 #9.*

The RTO is computed from a smoothed RTT estimate (SRTT) plus a multiple of the RTT variation (RTTVAR), per RFC 6298. Using only the most recent sample would make the timer too jittery — bursts of unusually fast or slow ACKs would cause spurious retransmissions or unnecessarily long waits before recovery.
...
> The outcome of the routing process is used to populate the forwarding tables at the routers

**Answer: True**

*Source: 2021 #11.*

Routing protocols (link-state, distance-vector, BGP) compute paths in the control plane and install the resulting next-hop entries into the forwarding table that the data plane consults on every packet. The split between control-plane routing and data-plane forwarding is a foundational concept in router architecture.
...
> Unlike in shared hubs, unicast frames are never flooded in switched Ethernet

**Answer: False**

*Source: 2021 #14.*

A switch floods a unicast frame on all ports (except the ingress port) when the destination MAC isn't in its forwarding table — for example, on the very first frame to a host the switch hasn't learned yet, or after a CAM table entry has aged out. So flooding does occur in switched Ethernet, just selectively rather than for every frame.
...
*Source: 2020 #17. Spring 2021 #15 asks the inverse phrasing ("would increase as well") and keys it True; both readings are consistent with the formula min_frame_size >= 2 * propagation_delay * bandwidth.*

Under that formula, increasing the cable length (max coverage) increases propagation delay, which means the minimum frame size needed for collision detection would *increase*, not decrease. So False is correct under the formula reading. (If you instead treat the minimum frame size as the fixed 64-byte standard value that doesn't change at all, this is a contested reading; the formula reading is what's being tested.)
...
*Source: 2020 #18 (key: False), 2021 #16 inverse phrasing (key: True).*

DISCREPANCY: The Spring 2020 answer key marks this False, but Spring 2021's companion question — "if the bandwidth is increased, the minimum frame size would increase as well" — is keyed True. Both questions are testing the same formula, min_frame_size >= 2 * propagation_delay * bandwidth, and under that formula:

- Increasing bandwidth requires a larger minimum frame size (2021's True is correct).
- Decreasing bandwidth allows a smaller minimum frame size (so this should also be True).

We follow the 2021 reading (True) here because it's internally consistent with the formula and with how question 21 (2020 #17) is graded. The 2020 key for this specific question appears to apply a different reading (treating min frame size as the fixed 64-byte standard, which would make it False) that contradicts its own grading of question 21.
...
> An Ethernet Adapter passes only non-corrupt frames that it receives up to the network layer

**Answer: True**

*Source: 2021 #20.*

The Ethernet NIC validates the frame check sequence (CRC-32) on every received frame and silently drops any whose checksum doesn't match. Only frames that pass CRC and match the adapter's MAC address (or a multicast/broadcast address it accepts) are handed up the stack to the network layer.

### 28

> If a TCP and a UDP flows share the same "bottleneck", the UDP flow is more likely will get a greater percentage of the bandwidth

**Answer: True**

*Source: 2021 #19. Spring 2020 #24 poses the same idea as a multiple-choice question (see question 30 below) with answer A.*

When a bottleneck saturates, TCP reacts to packet loss by halving its congestion window and slowing down. UDP has no congestion control and continues sending at its application rate. Over time, TCP's repeated backoff frees bandwidth that UDP captures, so UDP gets a larger share. This is the foundational result behind the "TCP-friendliness" concern in network design.
...
*Source: 2020 #24. Spring 2021 #19 poses the same idea as true/false (see question 28 above).*

When a bottleneck saturates, TCP reacts to packet loss by halving its congestion window and slowing down. UDP has no congestion control and continues sending at its application rate. Over time, TCP's repeated backoff frees bandwidth that UDP captures, so UDP gets a larger share.
